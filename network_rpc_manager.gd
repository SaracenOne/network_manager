...
func queue_reliable_rpc_call(p_entity : Node, p_method_id : int, p_args : Array):
	pending_rpc_reliable_calls.push_back({"entity":p_entity, "method_id":p_method_id, "args":p_args})

func queue_reliable_rset_call(p_entity : Node, p_property_id : int, p_value):
	pending_rset_reliable_calls.push_back({"entity":p_entity, "property_id":p_property_id, "value":p_value})

func queue_unreliable_rpc_call(p_entity : Node, p_method_id : int, p_args : Array):
	pending_rpc_unreliable_calls.push_back({"entity":p_entity, "method_id":p_method_id, "args":p_args})

func queue_unreliable_rset_call(p_entity : Node, p_property_id : int, p_value):
	pending_rset_unreliable_calls.push_back({"entity":p_entity, "property_id":p_property_id, "value":p_value})
...
func _network_manager_process(p_id : int, p_delta : float) -> void:
	if p_delta > 0.0:
		if pending_rpc_reliable_calls.size() > 0 or \
		pending_rpc_unreliable_calls.size() > 0 or \
		pending_rset_reliable_calls.size() > 0 or \
		pending_rset_unreliable_calls.size() > 0:
			...
			for synced_peer in synced_peers:
				...
				for call in pending_rpc_reliable_calls:
					var rpc_command_network_writer : network_writer_const = create_rpc_command(network_constants_const.ENTITY_RPC_COMMAND, call)
					network_reliable_writer_state.put_writer(rpc_command_network_writer, rpc_command_network_writer.get_position())
						
				for call in pending_rset_reliable_calls:
					var rset_command_network_writer : network_writer_const = create_rpc_command(network_constants_const.ENTITY_RSET_COMMAND, call)
					network_reliable_writer_state.put_writer(rset_command_network_writer, rset_command_network_writer.get_position())
				...
				if network_reliable_writer_state.get_position() > 0:
					var raw_data : PoolByteArray = network_reliable_writer_state.get_raw_data(network_reliable_writer_state.get_position())
				...
				if network_unreliable_writer_state.get_position() > 0:
					var raw_data : PoolByteArray = network_unreliable_writer_state.get_raw_data(network_unreliable_writer_state.get_position())
...
func decode_rpc_buffer(p_packet_sender_id : int, p_network_reader : network_reader_const, p_command : int) -> network_reader_const:
	match p_command:
		network_constants_const.ENTITY_RPC_COMMAND:
...
func _server_peer_connected(p_id : int) -> void:
	...
	var rpc_unreliable_writer : network_writer_const = network_writer_const.new(MAXIMUM_RPC_PACKET_SIZE)
...
func is_command_valid(p_command : int) -> bool:
	if p_command == network_constants_const.ENTITY_RPC_COMMAND or \
		p_command == network_constants_const.ENTITY_RSET_COMMAND:
		return true
	else:
		return false
